...
def test_voice_start_fails_without_stt(client: TestClient):
    with (
        patch("app.voice.assistant.VoiceAssistant._check_deps_ready", return_value=True),
        patch("app.voice.assistant.VoiceAssistant._check_stt_ready", return_value=False),
    ):
        response = client.post("/voice/start")
        assert response.status_code == 400
        detail = response.json()["detail"].lower()
        assert "faster-whisper" in detail or "ffmpeg" in detail or "speech-to-text" in detail
